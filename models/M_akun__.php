	...
	class M_akun extends CI_Model 
	{
		...
		function list_akun_limit($cari,$limit,$offset)
		{
			$query = $this->db->query("
										...
										FROM tb_akun AS A
										LEFT JOIN tb_karyawan AS B ON A.id_karyawan = B.id_karyawan A.kode_kantor = B.kode_kantor
		...
		function count_akun_limit($cari)
		{
			$query = $this->db->query("SELECT COUNT(id_akun) AS JUMLAH FROM tb_akun AS A
										...
										LEFT JOIN tb_jabatan AS C ON B.id_jabatan = C.id_jabatan AND
										A.kode_kantor = C.kode_kantor ".$cari);
			if($query->num_rows() > 0)
			{
				return $query->row();
			}
		...
		function get_hak_akses_group1($id_jabatan)
		{
			$query = $this->db->query('SELECT 
									...
									(
										...
										LEFT JOIN tb_hak_akses AS B ON A.id_jabatan = B.id_jabatan AND A.kode_kantor = B.kode_kantor
										LEFT JOIN tb_fasilitas AS C ON B.id_fasilitas = C.id_fasilitas AND A.kode_kantor = C.kode_kantor
