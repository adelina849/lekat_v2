...
class C_gl_pst_admin_supplier extends CI_Controller {
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$config['first_url'] = site_url('gl-pusat-admin-supplier?'.http_build_query($_GET));
				$config['base_url'] = site_url('gl-pusat-admin-supplier/');
				...
				$list_kat_supplier = $this->M_gl_kat_supplier->list_kat_supplier_limit(" WHERE kode_kantor = 'PST'",100,0);
				...
				$data = array('page_content'=>'gl_pusat_admin_supplier',
							  'halaman'=>$halaman,
							  'list_supplier'=>$list_supplier,
							  'msgbox_title' => $msgbox_title,
							  'no_spl' => $no_spl,
							  'list_kat_supplier' => $list_kat_supplier,
							  'sum_pesan' => $sum_pesan
							);
	...
	public function index_penerimaan_po()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$config['first_url'] = site_url('gl-pusat-admin-supplier-penerimaan-po?'.http_build_query($_GET));
				$config['base_url'] = site_url('gl-pusat-admin-supplier-penerimaan-po/');
				$config['total_rows'] = $jum_row;
				$config['uri_segment'] = 2;	
				$config['per_page'] = 30;
				$config['num_links'] = 2;
				...
				$data = array(
						'page_content'=>'gl_pusat_admin_supplier_penerimaan_po',
						'halaman'=>$halaman,
						'list_supplier'=>$list_supplier,
						'msgbox_title' => $msgbox_title,
						'no_spl' => $no_spl,
	...
	public function simpan()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				//echo $_POST['isPajak'];
				header('Location: '.base_url().'gl-pusat-admin-supplier');
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				if(!empty($data_supplier))
				{
					...
					$config['first_url'] = site_url('gl-pusat-admin-supplier-alias-produk/'.md5($data_supplier->id_supplier).'?'.http_build_query($_GET));
					$config['base_url'] = site_url('gl-pusat-admin-supplier-alias-produk/'.md5($data_supplier->id_supplier));
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				if(!empty($data_supplier))
				{
					...
					header('Location: '.base_url().'gl-pusat-admin-supplier-alias-produk/'.md5($data_supplier->id_supplier));
				}
				else
				{
					header('Location: '.base_url().'gl-pusat-admin-supplier');
	...
	public function alias_produk_supplier_hapus()
	{
		...
		header('Location: '.base_url().'gl-pusat-admin-supplier-alias-produk/'.$id_supplier);
	}
