...
class C_gl_admin_harga_dasar_satuan extends CI_Controller {
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$config['first_url'] = site_url('gl-admin-harga-dasar-satuan?'.http_build_query($_GET));
				$config['base_url'] = site_url('gl-admin-harga-dasar-satuan/');
				...
				$list_harga_dasar_satuan = $this->M_gl_satuan_konversi->list_harga_dasar_satuan($this->session->userdata('ses_kode_kantor'),$cari,$this->uri->segment(2,0),$config['per_page']);
				...
				$data = array('page_content'=>'gl_admin_harga_dasar_satuan','halaman'=>$halaman,'list_harga_dasar_satuan'=>$list_harga_dasar_satuan,'msgbox_title' => $msgbox_title,'list_field' => $list_field, 'sum_pesan' => $sum_pesan);
	...
	function simpan_old_konsep_pusat_update_ALL()
	{
		if($this->session->userdata('ses_kode_kantor') == 'PST')
		{
			...
			//$konversi = $_POST['konversi'];
				...
				$cek_satuan_konveri = $this->M_gl_pst_satuan_konversi->cek_satuan_konversi(" WHERE kode_kantor = (SELECT kode_kantor FROM tb_kantor WHERE isDefault = 1) AND id_produk = '".$id_produk."' AN id_satuan = '".$id_satuan."'");
				
				if(!empty($cek_satuan_konveri))
				{
					$cek_satuan_konveri = $cek_satuan_konveri->row();
					
					echo'Ubah harga dasar';
					...
					*/
					...
					$this->M_gl_pst_satuan_konversi->edit_by_idProduk_idSatuan(
														$id_produk,
														$id_satuan,
														$cek_satuan_konveri->besar_konversi,
														$harga_jual,
