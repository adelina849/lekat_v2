...
class C_gl_admin_peraturan extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('M_gl_peraturan'));
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$config['first_url'] = site_url('gl-admin-peraturan?'.http_build_query($_GET));
				$config['base_url'] = site_url('gl-admin-peraturan/');
				$config['total_rows'] = $this->M_gl_peraturan->count_peraturan_limit($cari)->JUMLAH;
				...
				$list_peraturan = $this->M_gl_peraturan->list_peraturan_limit($cari,$config['per_page'],$this->uri->segment(2,0));
				
				$msgbox_title = " Peraturan Perusahaan";
				
				$data = array('page_content'=>'gl_admin_peraturan','halaman'=>$halaman,'list_peraturan'=>$list_peraturan,'msgbox_title' => $msgbox_title);
	...
	public function simpan()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				header('Location: '.base_url().'gl-admin-peraturan');
			}
			else
			{
				header('Location: '.base_url().'gl-admin-login');
	...
	public function hapus()
	{
		...
		$data_peraturan = $this->M_gl_peraturan->get_peraturan('id_per',$id_per);
		if(!empty($data_peraturan))
		{
			$data_peraturan = $data_peraturan->row();
			$this->M_gl_peraturan->hapus($id_per);
