...
class C_gl_pst_admin_uang_masuk extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('M_gl_uang_masuk','M_gl_pst_uang_masuk','M_gl_bank','M_gl_kode_akun','M_gl_pengaturan','M_gl_pst_uang_masuk'));
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$jum_row = $this->M_gl_pst_uang_masuk->count_uang_masuk_limit($cari)->JUMLAH;
				
				$this->load->library('pagination');
				//$config['first_url'] = base_url().'admin/jabatan?'.http_build_query($_GET);
				//$config['base_url'] = base_url().'admin/jabatan/';
				
				$config['first_url'] = site_url('gl-pusat-admin-pemasukan?'.http_build_query($_GET));
				$config['base_url'] = site_url('gl-pusat-admin-pemasukan/');
				...
				/*
				$get_no_uang_masuk = $this->M_gl_pst_uang_masuk->get_no_uang_masuk($kode_kantor);
				...
				*/
				...
				$list_uang_masuk = $this->M_gl_pst_uang_masuk->list_uang_masuk_limit_untuk_induk($cari,$order_by,$config['per_page'],$this->uri->segment(2,0));
				...
				//SUMAARY LIST
					$sum_uang_masuk = $this->M_gl_pst_uang_masuk->sum_uang_masuk_limit($cari);
				...
				$cari_kode_akun_from_lap = "WHERE kode_kantor = '".$kode_kantor."' AND kode_akun = '".$kode_from_lap."' ";
				...
				$data = array('page_content'=>'c_gl_akuntansi/gl_admin_uang_masuk',
							  'halaman'=>$halaman,
							  'list_uang_masuk'=>$list_uang_masuk,
							  'msgbox_title' => $msgbox_title,
							  'sum_pesan' => $sum_pesan,
							  'list_bank'=>$list_bank,
							  //'get_no_uang_masuk'=>$get_no_uang_masuk,
							  'sum_uang_masuk'=>$sum_uang_masuk,
							  'list_kode_akun' => $list_kode_akun,
							  'list_kantor' => $list_kantor,
							  'get_kode_akun_from_lap' => $get_kode_akun_from_lap,
							  'kode_from_lap' => $kode_from_lap
							);
	...
	function get_no_uang_masuk_percab()
	{
		
		//DAPATKAN NO UANG MASUK
		$get_no_uang_masuk = $this->M_gl_pst_uang_masuk->get_no_uang_masuk($_POST['kode_kantor']);
		if(!empty($get_no_uang_masuk))
		{
			$get_no_uang_masuk = $get_no_uang_masuk->row();
			$get_no_uang_masuk = $get_no_uang_masuk->no_bukti;
	...
	public function list_tarik_transaksi_ke_akun()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$data = array('page_content'=>'gl_pst_admin_transfer_ke_akun','halaman'=>$halaman,'list_uang_masuk'=>$list_uang_masuk,'msgbox_title' => $msgbox_title,'sum_pesan' => $sum_pesan,'list_bank'=>$list_bank,'get_no_uang_masuk'=>$get_no_uang_masuk,'sum_uang_masuk'=>$sum_uang_masuk,'list_kode_akun' => $list_kode_akun,'list_kantor' => $list_kantor,'list_transfer_to_akun' => $list_transfer_to_akun);
				$this->load->view('pusat/container',$data);
			}
			else
			{
	...
	public function list_kode_akun_lap_keuangan()
	{
		$cari_coa = "WHERE kode_kantor = '".$this->input->post('kode_kantor')."' AND target IN ('KAS','KAS-BESAR','BANK','CASHIN-TRANSIT')";
		$order_by_coa = " ORDER BY nama_kode_akun ASC ";
		$list_kode_akun = $this->M_gl_kode_akun->list_kode_akun_limit_order_by($cari_coa,$order_by_coa,1000,0);
	...
	public function tambah_pemasukan()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$get_data_uang_masuk = $this->M_gl_pst_uang_masuk->list_uang_masuk_limit($cari_uang_masuk,"",1,0);
				if(!empty($get_data_uang_masuk))
				{
					...
					$config['first_url'] = site_url('gl-pusat-admin-pemasukan-tambah/'.$id_uang_masuk.'??'.http_build_query($_GET));
					$config['base_url'] = site_url('gl-pusat-admin-pemasukan-tambah/'.$id_uang_masuk);
					$config['total_rows'] = $jum_row;
					...
					//DAPATKAN NO UANG MASUK
					$get_no_uang_masuk = $this->M_gl_pst_uang_masuk->get_no_uang_masuk($this->session->userdata('ses_kode_kantor'));
					if(!empty($get_no_uang_masuk))
					{
						$get_no_uang_masuk = $get_no_uang_masuk->row();
						$get_no_uang_masuk = $get_no_uang_masuk->no_bukti;
					...
					$list_uang_masuk = $this->M_gl_pst_uang_masuk->list_uang_masuk_limit($cari,$order_by,$config['per_page'],$this->uri->segment(3,0));
				...
				{
					header('Location: '.base_url().'gl-pusat-admin-pemasukan');
				}
			}
			else
			{
	...
	function tampilkan_pemasukan_tambah()
	{
		...
		$list_uang_masuk = $this->M_gl_pst_uang_masuk->list_uang_masuk_limit($cari,$order_by,1000,0);
...
<i class="fa fa-trash"></i> HAPUS
	...
	{
		$this->M_gl_pst_uang_masuk->simpan
		(
...
<i class="fa fa-trash"></i> HAPUS
	...
	public function hapus_ajax()
	{
		...
		$data_uang_masuk = $this->M_gl_pst_uang_masuk->get_uang_masuk('(id_uang_masuk)',$id_uang_masuk,$kode_kantor);
		if(!empty($data_uang_masuk))
		{
			$data_uang_masuk = $data_uang_masuk->row();
			$this->M_gl_pst_uang_masuk->hapus('(id_uang_masuk)',$id_uang_masuk,$kode_kantor);
...
<i class="fa fa-trash"></i> HAPUS
	...
	public function print_bukti_uang_masuk()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$cari_uang_masuk = "WHERE A.kode_kantor = '".$kode_kantor."' AND (A.id_uang_masuk) = '".$id_uang_masuk."'";
				$get_uang_masuk = $this->M_gl_pst_uang_masuk->list_uang_masuk_limit($cari_uang_masuk,"",1,0);
				if(!empty($get_uang_masuk))
				{
					...
					$list_uang_masuk = $this->M_gl_pst_uang_masuk->list_uang_masuk_limit($cari,$order_by,100,0);
					...
					//$this->load->view('pusat/container',$data);
					$this->load->view('pusat/page/c_gl_akuntansi/gl_admin_print_bukti_uang_masuk.html',$data);
				}
				else
				{
					header('Location: '.base_url().'gl-pusat-admin-pemasukan');
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				else
				{
					...
					{
						//$this->M_gl_pst_uang_masuk->simpan_utama
				...
				if((!empty($_POST['id_induk_uang_masuk'])) && ($_POST['id_induk_uang_masuk']!= "")  )
				{
					...
					{
						header('Location: '.base_url().'gl-pusat-admin-transfer-ke-akun?stat_edit=&kode_kantor='.$_POST['cari_kode_kantor'].'&tgl_uang_masuk='.$_POST['tgl_uang_masuk'].'');
					}
					else
					{
						header('Location: '.base_url().'gl-pusat-admin-pemasukan-tambah/'.md5($_POST['id_induk_uang_masuk']));
					}
				}
				else
				{
	...
	public function hapus()
	{
		...
		$data_uang_masuk = $this->M_gl_pst_uang_masuk->get_uang_masuk('MD5(id_uang_masuk)',$id_uang_masuk,$kode_kantor);
		if(!empty($data_uang_masuk))
		{
			$data_uang_masuk = $data_uang_masuk->row();
			$this->M_gl_pst_uang_masuk->hapus('MD5(id_induk_uang_masuk)',$id_uang_masuk,$kode_kantor);
	...
	{
		$hasil_cek = $this->M_gl_pst_uang_masuk->get_uang_masuk('no_bukti',$_POST['no_bukti']);
		if(!empty($hasil_cek))
		{
