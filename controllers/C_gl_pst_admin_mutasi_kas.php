...
class C_gl_pst_admin_mutasi_kas extends CI_Controller {
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$config['first_url'] = site_url('gl-pusat-admin-mutasi-kas?'.http_build_query($_GET));
				$config['base_url'] = site_url('gl-pusat-admin-mutasi-kas/');
				...
				/*
				$get_no_bukti = $this->M_gl_mutasi_kas->get_no_bukti($kode_kantor);
				...
				*/
				...
				$data = array('page_content'=>'c_gl_akuntansi/gl_admin_mutasi_kas',
							  'halaman'=>$halaman,
							  'list_mutasi_kas'=>$list_mutasi_kas,
							  'msgbox_title' => $msgbox_title,
							  //'list_kode_akun' => $list_kode_akun,
							  'get_no_bukti' => $get_no_bukti,
	...
	function get_no_mutasi_kas()
	{
		$get_no_bukti = $this->M_gl_mutasi_kas->get_no_bukti($_POST['kode_kantor']);
		$get_no_bukti = $get_no_bukti->row();
		$get_no_bukti = $get_no_bukti->no_bukti;
		echo $get_no_bukti;
	}
	...
	public function print_bukti_mutasi_kas()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$cari_mutasi = " WHERE A.kode_kantor = '".$kode_kantor."' AND (A.id_mutasi) = '".$id_mutasi."' ";
				...
				if(!empty($data_mutasi_kas))
				{
					...
					$data = array('page_content'=>'c_gl_akuntansi/gl_admin_print_mutasi_kas','mutasi_kas' => $mutasi_kas);
					
					//$this->load->view('pusat/container',$data);
					$this->load->view('pusat/page/c_gl_akuntansi/gl_admin_print_mutasi_kas.html',$data);
				}
				else
				{
					header('Location: '.base_url().'gl-pusat-admin-mutasi-kas');
				}
			}
			else
			{
	...
	public function hapus()
	{
		...
		$cari_mutasi = " WHERE kode_kantor = '".$kode_kantor."' AND MD5(id_mutasi) = '".$id_mutasi."' ";
		$data_mutasi_kas = $this->M_gl_mutasi_kas->get_mutasi_cari($cari_mutasi);
		if(!empty($data_mutasi_kas))
		{
			$data_mutasi_kas = $data_mutasi_kas->row();
			$this->M_gl_mutasi_kas->hapus('MD5(id_mutasi)',$id_mutasi,$kode_kantor);
