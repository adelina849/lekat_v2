			...
			if(!empty($cek_ses_login))
			{
				...
				if((!empty($_GET['cari'])) && ($_GET['cari']!= "")  )
				{
					$cari = "
							...
							AND (
								B.kode_kantor_mutasi LIKE '%".str_replace("'","",$_GET['cari'])."%' 
	...
	public function list_mutasi_dari_cabang_lain_forus()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				{
					$cari = "
							WHERE B.kode_kantor_mutasi = '".$this->session->userdata('ses_kode_kantor')."' AND COALESCE(DATE(B.tgl_h_penjualan),'') BETWEEN '".$dari."' AND '".$sampai."'
							-- AND COALESCE(B.status_penjualan,'') LIKE '%".$jenis_mutasi."%'
							AND COALESCE(B.status_penjualan,'') = 'MUTASI-OUT'
							AND (
				...
				{
					$cari = "
						WHERE B.kode_kantor_mutasi = '".$this->session->userdata('ses_kode_kantor')."' 
						AND COALESCE(DATE(B.tgl_h_penjualan),'') BETWEEN '".$dari."' AND '".$sampai."' 
						-- AND COALESCE(B.status_penjualan,'') LIKE '%".$jenis_mutasi."%'
						AND COALESCE(B.status_penjualan,'') = 'MUTASI-OUT'
						";
				...
				$data = array('page_content'=>'gl_admin_lap_h_mutasi_dari_cabang_lain_forus','list_h_mutasi'=>$list_h_mutasi,'msgbox_title' => $msgbox_title);
				$this->load->view('admin/container',$data);
			}
			else
			{
