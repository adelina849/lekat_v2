...
class C_admin_jadwal_jemput_zakat extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		// Your own constructor code
		$this->load->model(array('M_jadwal_jemput_zakat','M_karyawan'));
	}
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				{
				$cari = "WHERE A.JMPT_KODEKANTOR = '".$this->session->userdata('ses_kode_kantor')."' 
						AND (B.nama_karyawan LIKE '%".str_replace("'","",$_GET['cari'])."%')";
				}
				else
				{
				$cari = "WHERE A.JMPT_KODEKANTOR = '".$this->session->userdata('ses_kode_kantor')."'";
				...
				$config['first_url'] = site_url('admin-jadwal-jemput-zakat?'.http_build_query($_GET));
				$config['base_url'] = site_url('admin-jadwal-jemput-zakat/');
				$config['total_rows'] = $this->M_jadwal_jemput_zakat->count_jemput_zakat_limit($cari)->JUMLAH;
				...
				$list_karyawan = $this->M_karyawan->list_karyawan_limit('',10,0);
				$list_jadwal_jemput_zakat = $this->M_jadwal_jemput_zakat->list_jemput_zakat_limit($cari,$config['per_page'],$this->uri->segment(2,0));
				
				$data = array('page_content'=>'king_admin_jadwal_jemput_zakat','list_karyawan' => $list_karyawan,'list_jadwal_jemput_zakat' => $list_jadwal_jemput_zakat,'halaman'=>$halaman);
	...
	public function simpan()
	{
		...
		{
			$this->M_jadwal_jemput_zakat->edit
			(
		...
		{
			$this->M_jadwal_jemput_zakat->simpan
			(
	...
	function cek_table_karyawan_ALL()
	{
		if((!empty($_POST['cari'])) && ($_POST['cari']!= "")  )
		{
			$cari = " WHERE  A.kode_kantor = '".$this->session->userdata('ses_kode_kantor')."' 
					AND (A.nama_karyawan LIKE '%".$_POST['cari']."%' OR A.no_karyawan LIKE '%".$_POST['cari']."%') ";
		}
		else
		{
			$cari= " WHERE A.kode_kantor = '".$this->session->userdata('ses_kode_kantor')."' ";
