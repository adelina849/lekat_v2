...
class C_kec_profile extends CI_Controller {
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$data = array('page_content' => 'ptn_kec_profile','data_karyawan' => $data_karyawan);
				$this->load->view('kecamatan/container',$data);
	...
	public function ubah_password()
	{
		if($_POST['pass'] == $_POST['pass2'])
		{
			...
			$pass = $_POST['pass'];
			$this->M_akun->edit_password($id_karyawan,$pass);
			
			header('Location: '.base_url().'C_kec_login/logout');
		}
		else
		{
			header('Location: '.base_url().'kec-profile/pass');
		}
	}
	...
	public function simpan()
	{
		...
		else
		{
			$no_karyawan = $_POST['no_karyawan'];
				...
				header('Location: '.base_url().'kec-profile');
