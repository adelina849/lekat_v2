...
class C_gl_admin_h_pembelian extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('M_gl_h_pembelian','M_gl_d_pembelian','M_gl_produk','M_gl_satuan_konversi','M_gl_supplier','M_gl_bank','M_gl_d_pembelian_bayar','M_gl_kat_produk','M_gl_lap_pembelian'));
	...
	function list_h_pembelian_penerimaan()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$config['first_url'] = site_url('gl-admin-purchase-order-terima?'.http_build_query($_GET));
				$config['base_url'] = site_url('gl-admin-purchase-order-terima/');
				...
				$data = array('page_content'=>'gl_admin_h_pembelian_penerimaan','halaman'=>$halaman,'list_h_pembelian_penerimaan'=>$list_h_pembelian_penerimaan,'msgbox_title' => $msgbox_title,'sum_pesan' => $sum_pesan,'list_data_supplier' => $list_data_supplier);
				$this->load->view('admin/container',$data);
			}
			else
			{
			...
			if(!empty($cek_ses_login))
			{
				...
				*/
				...
				$cari = "
							...
							AND A.id_h_pembelian = '".$_POST['id_h_pembelian']."'
							-- AND A.jumlah > COALESCE(D.TERIMA,0)
							";
				
				$order_by = "ORDER BY A.tgl_ins DESC";
				
				$list_view_d_pembelian = $this->M_gl_lap_pembelian->list_lap_d_pembelian_with_penerimaan_dipakai_laporan($cari,$order_by,10000,0);
...
<tr>';
							...
							foreach($list_result as $row)
							{
								echo'<tr>';
									...
									*/
									if($row->jumlah <> $row->TERIMA)
									{
										echo '<td style="color:red;">'.number_format($row->TERIMA,0,'.',',').' '.$row->kode_satuan.'</td>';
									}
									else
									{
										echo '<td>'.number_format($row->TERIMA,0,'.',',').' '.$row->kode_satuan.'</td>';
									}
	...
	function list_tgl_expired()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$list_h_pembelian_penerimaan_tgl_exp = $this->M_gl_h_pembelian->list_tgl_expired_with_stock($kode_kantor,$this->session->userdata('ses_gnl_tglStockProduk'),$cari);
...
<tr>';
					...
					echo'<tr id="tr_list_produk-'.$no.'">';
						...
						if($row->isProduk == 'CONSUMABLE')
						{
							echo'<td>
								...
								<br/> <b>HARGA : </b><b style="color:red;">'.number_format($row->harga_dasar,2,'.',',').' /'.$row->kode_satuan.'</b>
								<br/> <b>JENIS : </b><font style="color:blue;">'.$row->isProduk.'</font> 
								<br/> <b>SUPPLIER : </b><font style="color:blue;">'.$row->supplier.'</font> 
							</td>';
						}
						else
						{
						...
						echo'<input type="hidden" id="harga_jual_3_'.$no.'" name="harga_jual_3_'.$no.'" value="'.$row->harga_dasar.'" />';
	...
	function combo_list_satuan()
	{
		$cari = " WHERE A.kode_kantor = '".$this->session->userdata('ses_kode_kantor')."' 
				...
				AND COALESCE(B.kode_satuan,'') <> '".$_POST['kode_satuan_old']."'
				AND COALESCE(B.nama_satuan,'') <> '".$_POST['kode_satuan_old']."'
				";
		...
		{
			echo '<option value="'.$_POST['kode_satuan_old'].'">'.$_POST['kode_satuan_old'].'</option>';
		...
		if(!empty($list_d_pembelian))
		{
				...
				foreach($list_result as $row)
				{
					...
					echo'<tr id="tr_list_transaksi-'.$row->id_produk.'">';
						...
						echo'<td>'.$row->kode_produk.'</td>';
						echo'<td>'.$row->nama_produk.'</td>';
						echo'<td><input type="text" name="jumlah-'.$row->id_produk.'" id="jumlah-'.$row->id_produk.'" maxlength="6" size="6" onkeypress="return isNumberKey(event)" onfocusout="getRupiah(this.id)" onchange="edit_d_pembelian(this.id)" class="required " value="'.$row->jumlah.'" /></td>';
