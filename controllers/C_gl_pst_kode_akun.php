...
class C_gl_pst_kode_akun extends CI_Controller {
	...
	public function index()
	{
		...
		else
		{
			...
			{
				if((!empty($_GET['cari_kode_kantor'])) && ($_GET['cari_kode_kantor']!= "")  )
				{
					$kode_kantor = str_replace("'","",$_GET['cari_kode_kantor']);
				}
				else
				{
					$kode_kantor = '';
				...
				$config['first_url'] = site_url('gl-pusat-kode-akuntansi?'.http_build_query($_GET));
				$config['base_url'] = site_url('gl-pusat-kode-akuntansi/');
				...
				$data = array('page_content'=>'gl_pusat_kode_akun','halaman'=>$halaman,'list_kode_akun'=>$list_kode_akun,'msgbox_title' => $msgbox_title,'list_kode_akun_induk'=>$list_kode_akun_induk,'list_bank' => $list_bank,'list_supplier' => $list_supplier,'list_kantor' => $list_kantor);
	...
	function simpan_test()
	{
		echo $_POST['kat_akun_jurnal'];
	}
			...
			if(!empty($cek_ses_login))
			{
				...
				else
				{
					...
					(
						...
						$_POST['isLabaRugi'],
						$this->session->userdata('ses_id_karyawan'),
						$_POST['kode_kantor']//$this->session->userdata('ses_kode_kantor')
					);
				}
				header('Location: '.base_url().'gl-pusat-kode-akuntansi?cari_kode_kantor='.$_POST['kode_kantor']);
			}
			else
			{
				header('Location: '.base_url().'gl-admin-login');
	...
	public function hapus()
	{
		...
		$data_kode_akun = $this->M_gl_kode_akun->get_kode_akun_by_cari(" WHERE md5(kode_kantor) = '".$kode_kantor."' AND id_kode_akun = '".$id_kode_akun."' ");
		if(!empty($data_kode_akun))
		{
			$data_kode_akun = $data_kode_akun->row();
			$this->M_gl_kode_akun->hapus_pusat(" WHERE md5(kode_kantor) = '".$kode_kantor."' AND id_kode_akun = '".$id_kode_akun."' ");
			...
			header('Location: '.base_url().'gl-pusat-kode-akuntansi?cari_kode_akun='.$data_kode_akun->kode_kantor);
		}
	...
	{
		$hasil_cek = $this->M_gl_kode_akun->get_kode_akun_by_cari("WHERE target = '".$_POST['nilai']."'");
		if(!empty($hasil_cek))
		{
			$hasil_cek = $hasil_cek->row();
			echo $hasil_cek->kode_akun;
