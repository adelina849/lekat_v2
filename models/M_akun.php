<?php
	class M_akun extends CI_Model 
	{
		...
		function list_catatan_laporan($cari)
		{
			$query = "
						SELECT
							C.KEC_ID,C.LAP_ID,C.BLAP_CATATAN
							,COALESCE(B.LAP_NAMA,'') AS LAP_NAMA
							,COALESCE(A.KEC_NAMA,'') AS KEC_NAMA
						FROM 
						tb_buat_laporan AS C
						LEFT JOIN tb_laporan AS B ON C.LAP_ID = B.LAP_ID
						LEFT JOIN tb_kec AS A ON C.KEC_ID = A.KEC_ID
						WHERE (COALESCE(B.LAP_NAMA,'') LIKE '%".$cari."%' OR COALESCE(A.KEC_NAMA,'') LIKE '%".$cari."%') 
						AND C.BLAP_PERIODE IN (
											SELECT PER_KODE 
											FROM tb_periode
											WHERE NOW() >= PER_DARI  AND NOW() <= PER_SAMPAI
											)
						AND (C.BLAP_CATATAN <> '' AND C.BLAP_CATATAN IS NOT NULL)
						GROUP BY C.LAP_ID,C.BLAP_CATATAN,COALESCE(B.LAP_NAMA,''),COALESCE(A.KEC_NAMA,'')
					";
		...
		function hitung_catatan($KEC_ID)
		{
			$query = "
						SELECT COUNT(BLAP_CATATAN) AS CNT
						FROM
						(
							SELECT
								LAP_ID,BLAP_CATATAN
							FROM 
							tb_buat_laporan AS C 
							WHERE C.KEC_ID LIKE '%".$KEC_ID."%' 
							AND BLAP_PERIODE IN (
												SELECT PER_KODE 
												FROM tb_periode
												WHERE NOW() >= PER_DARI  AND NOW() <= PER_SAMPAI
												)
							AND (BLAP_CATATAN <> '' AND BLAP_CATATAN IS NOT NULL)
							GROUP BY LAP_ID,BLAP_CATATAN
						) AS BB
						;
		...
		function get_no_karyawan($kode_kantor)
		{
			$query = $this->db->query(
			"
		...
		function edit_password($id_karyawan,$pass)
		{
			$query = "UPDATE tb_akun SET pass = '".md5($pass)."'  WHERE id_karyawan = '".$id_karyawan."' ";
			$this->db->query($query);
		}
		...
		function get_login_kabupaten($user,$pass)
        {
            //$query = $this->db->get_where('tb_akun', array('user' => $user),'pass'=> $pass);
			$query = $this->db->query("
				SELECT A.* 
					...
					,COALESCE(D.KEC_KODE,'') AS KEC_KODE
					,COALESCE(D.KEC_NAMA,'') AS KEC_NAMA
					,COALESCE(D.KEC_CAMAT,'') AS KEC_CAMAT
					,COALESCE(D.KEC_ALAMAT,'') AS KEC_ALAMAT
				FROM tb_akun AS A
				LEFT JOIN tb_karyawan AS B ON A.id_karyawan = B.id_karyawan -- AND A.kode_kantor = B.kode_kantor
				LEFT JOIN tb_jabatan AS C ON B.id_jabatan = C.id_jabatan -- AND A.kode_kantor = C.kode_kantor
				LEFT JOIN tb_kec AS D ON B.KEC_ID = D.KEC_ID
				WHERE B.status_kantor = 'KAB' AND A.user = '".$user."' AND A.pass = '".$pass."'");
		...
		function list_klaporan()
		{
			$query = $this->db->query(
			"
				SELECT * FROM tb_klaporan ORDER BY KLAP_NAMA
			"
			);
		...
		function list_laporan($KLAP_ID)
		{
			$query = $this->db->query(
			"
				SELECT * FROM tb_laporan WHERE KLAP_ID = '".$KLAP_ID."' ORDER BY LAP_NAMA
			"
			);
