<?php
	class M_gl_pst_punish extends CI_Model 
	{
		...
		function hapus($berdasarkan,$id_punish,$kode_kantor)
		{
			/*HAPUS JABATAN*/
				$strquery = "DELETE FROM tb_punish WHERE ".$berdasarkan." = '".$id_punish."' AND kode_kantor = '".$kode_kantor."' ;";
			/*HAPUS JABATAN*/
		...
		function get_punish($berdasarkan,$cari,$kode_kantor)
        {
            $query = $this->db->get_where('tb_punish', array($berdasarkan => $cari,'kode_kantor' => $kode_kantor));
            if($query->num_rows() > 0)
            {
                return $query;
            }
