<?php
	class M_jabatan extends CI_Model 
	{
		...
		{
			$query = $this->db->query("SELECT * FROM tb_jabatan ORDER BY nama_jabatan ASC");
			if($query->num_rows() > 0)
			{
				return $query;
			}
		...
		function hapus($id)
		{
			$this->db->query('DELETE FROM tb_jabatan WHERE id_jabatan = '.$id.'');
		}
		
		function get_jabatan_num_rows($berdasarkan,$cari)
        {
            $query = $this->db->get_where('tb_jabatan', array($berdasarkan => $cari));
            if($query->num_rows() > 0)
            {
