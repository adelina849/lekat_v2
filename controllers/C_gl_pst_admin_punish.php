...
class C_gl_pst_admin_punish extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		// Your own constructor code
		$this->load->model(array('M_gl_pst_punish','M_gl_karyawan','M_gl_peraturan','M_gl_pengaturan'));
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				{
					$cari = "WHERE  A.kode_kantor LIKE '%".$kode_kantor."%'
				...
				$config['first_url'] = site_url('gl-pusat-admin-punishment?'.http_build_query($_GET));
				$config['base_url'] = site_url('gl-pusat-admin-punishment/');
				$config['total_rows'] = $this->M_gl_pst_punish->count_punish_limit($cari)->JUMLAH;
				...
				$list_punish = $this->M_gl_pst_punish->list_punish_limit($cari,$config['per_page'],$this->uri->segment(2,0));
				$list_kantor = $this->M_gl_pengaturan->get_data_kantor("")->result();
				
				$msgbox_title = " Punishment Karyawan";
				
				$data = array(
						'page_content'=>'gl_pusat_admin_punish',
						'halaman'=>$halaman,
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				{	
					...
					//echo 'edit kode_kantor '.$_POST['kode_kantor_p'];
				...
				header('Location: '.base_url().'gl-pusat-admin-punishment');
	...
	public function hapus()
	{
		...
		$data_punish = $this->M_gl_pst_punish->get_punish('id_punish',$id_punish,$kode_kantor);
		if(!empty($data_punish))
		{
			$data_punish = $data_punish->row();
			
			//HAPUS TRAINING
				$this->M_gl_pst_punish->hapus('id_punish',$id_punish,$kode_kantor);
			//HAPUS TRAINING
	...
	function list_karyawan()
	{
		...
		{
			$cari = "WHERE A.isDefault = '1' AND (COALESCE(A.no_karyawan,'') LIKE '%".$_POST['cari']."%' OR COALESCE(A.nama_karyawan,'') LIKE '%".$_POST['cari']."%')
							";
		}
		else
		{
			$cari = "WHERE A.isDefault = '1'";
		}
			...
			echo'<table width="100%" id="example2" class="table table-hover">';
				...
				foreach($list_result as $row)
				{
					echo'<tr id="tr_'.$no.'">';
						...
						echo'<input type="hidden" id="kode_kantor_3_'.$no.'" name="kode_kantor_3_'.$no.'" value="'.$row->kode_kantor.'" />';
					echo'</tr>';
					$no++;
				}
...
<tr>';
	...
	function cek_punish()
	{
		$hasil_cek = $this->M_gl_pst_punish->get_punish('kode_promosi',$_POST['kode_promosi']);
		echo $hasil_cek;
	}
	
	function ubah_proses_karyawan_promo()
	{
		$hasil_cek = $this->M_gl_pst_punish->edit_ubah_nilai($this->session->userdata('ses_kode_kantor'),$_POST['id_kj'],$_POST['nilai'],$_POST['status'],$_POST['ket_nilai']);
