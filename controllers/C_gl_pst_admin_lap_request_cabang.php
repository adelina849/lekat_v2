...
class C_gl_pst_admin_lap_request_cabang extends CI_Controller {
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				//$config['first_url'] = site_url('gl-pusat-admin-laporan-transaksi?'.http_build_query($_GET));
				$config['first_url'] = site_url('gl-pusat-admin-laporan-permintaan-cabang?'.http_build_query($_GET));
				$config['base_url'] = site_url('gl-pusat-admin-laporan-permintaan-cabang/');
				...
				$data = array(
						'page_content'=>'gl_pusat_admin_lap_req_cabang',
						'halaman'=>$halaman,
						'sum_pesan' => $sum_pesan,
						'msgbox_title' => $msgbox_title,
						'list_pemesanan_dari_cabang_lain' => $list_pemesanan_dari_cabang_lain,
					);
				...
				if(!empty($get_data_pembelian))
				{
					...
					if(!empty($get_data_costumer))
					{
						...
						else
						{
							...
							//header('Location: '.base_url().'gl-pusat-admin-laporan-permintaan-cabang');
						}
					}
					else
					{
