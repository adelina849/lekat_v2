...
class C_gl_pst_kat_produk extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('M_gl_pst_kat_produk'));
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$config['first_url'] = site_url('gl-pusat-kategori-produk-jasa?'.http_build_query($_GET));
				$config['base_url'] = site_url('gl-pusat-kategori-produk-jasa/');
				$config['total_rows'] = $this->M_gl_pst_kat_produk->count_kat_produk_limit($cari)->JUMLAH;
				...
				$list_kat_produk = $this->M_gl_pst_kat_produk->list_kat_produk_limit($cari,$config['per_page'],$this->uri->segment(2,0));
				
				$msgbox_title = " Kategori Produk Dan Jasa";
				
				$data = array('page_content'=>'gl_pusat_kat_produk','halaman'=>$halaman,'list_kat_produk'=>$list_kat_produk,'msgbox_title' => $msgbox_title);
	...
	public function simpan()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				{	
					...
					if($this->session->userdata('catat_log') == 'Y')
					{
						...
						(
							$this->session->userdata('ses_id_karyawan'),
							'UPDATE',
							'Melakukan perubahan data Kategori Produk dan Jasa '.$_POST['kode_kat_produk'].' | '.$_POST['nama_kat_produk'],
				...
				header('Location: '.base_url().'gl-pusat-kategori-produk-jasa');
			}
			else
			{
				header('Location: '.base_url().'gl-admin-login');
