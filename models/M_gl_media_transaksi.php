<?php
	class M_gl_media_transaksi extends CI_Model 
	{
		...
		)
		{
			$strquery = "
				...
				(
					(
						SELECT CONCAT('".$this->session->userdata('isLocal')."','".$kode_kantor."',FRMTGL,ORD) AS id_tr_media
						From
						(
		...
		)
		{
			$strquery = "
					...
					WHERE kode_kantor = '".$kode_kantor."' AND id_tr_media = '".$id_tr_media
					."'
					";
		...
		{
			/*HAPUS JABATAN*/
				$strquery = "DELETE FROM tb_media_transaksi WHERE ".$berdasarkan." = '".$id_tr_media."' AND kode_kantor = '".$this->session->userdata('ses_kode_kantor')."' ;";
			/*HAPUS JABATAN*/
        ...
        {
            $query = $this->db->get_where('tb_media_transaksi', array($berdasarkan => $cari,'kode_kantor' => $this->session->userdata('ses_kode_kantor')));
            if($query->num_rows() > 0)
            {
                return $query;
            }
		...
		function set_default($id_tr_media,$kode_kantor)
		{
				/*HAPUS JABATAN*/
					$strquery = 
					"
