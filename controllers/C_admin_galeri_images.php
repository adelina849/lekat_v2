...
class C_admin_galeri_images extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		// Your own constructor code
		$this->load->model(array('M_images','M_kat_images'));
	}
		...
		else
		{
			...
			{	
				...
				if(!empty($JUMLAH_RECORD))
				{
					...
					$config['first_url'] = site_url('admin-galeri-images?'.http_build_query($_GET));
					$config['base_url'] = site_url('admin-galeri-images');
					...
					$list_kat_images = $this->M_kat_images->list_kat_images_limit('',100,0);
					
					$data = array('page_content'=>'king_admin_galeri_images','halaman'=>$halaman,'list_images'=>$list_images,'list_kat_images' => $list_kat_images);
	...
	public function simpan()
	{
			if (!empty($_POST['stat_edit']))
			{
				...
				header('Location: '.base_url().'admin-galeri-images');
			}
			else
			{
				if (!empty($_FILES['foto']['name']))
