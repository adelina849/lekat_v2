...
class C_gl_pst_harga_pasien extends CI_Controller {
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$jum_row = $this->M_gl_harga_pasien->count_harga_pasien_pusat($kode_kantor,$cari)->JUMLAH;
				
				$this->load->library('pagination');
				
				$config['first_url'] = site_url('gl-pusat-harga-pasien?'.http_build_query($_GET));
				$config['base_url'] = site_url('gl-pusat-harga-pasien/');
				...
				$list_harga_pasien = $this->M_gl_harga_pasien->list_harga_pasien($kode_kantor,$cari,$this->uri->segment(2,0),$config['per_page']);
				...
				$data = array('page_content'=>'gl_pusat_harga_pasien','halaman'=>$halaman,'list_harga_pasien'=>$list_harga_pasien,'msgbox_title' => $msgbox_title,'list_field' => $list_field, 'sum_pesan' => $sum_pesan);
	...
	function simpan()
	{
		...
		$cari_kat_costumer = "WHERE kode_kantor = (SELECT kode_kantor FROM tb_kantor WHERE isDefault = 1) AND nama_kat_costumer = '".$_POST['strKatCos']."'";
		$id_kat_costumer = $this->M_gl_kat_costumer->get_kat_costumer_cari($cari_kat_costumer);
		...
		if($harga > 0)
		{
			$cek_harga_satuan_costumer = $this->M_gl_harga_pasien->cek_harga_satuan_costumer("  WHERE kode_kantor = (SELECT kode_kantor FROM tb_kantor WHERE isDefault = 1) AND id_produk = '".$id_produk."' AND id_satuan = '".$id_satuan."' AND id_costumer = '".$id_kat_costumer."' AND media = '".$media."' ");
